...
urlpatterns = patterns('',                       
    ...
    url(r'^application/rejected/list/$', ListView.as_view(queryset=Application.objects.filter(approved=False, reviewed=True), 
                                  context_object_name='rejected_list', template_name='consumer/application_list.html'), name='rejected-list'),
    ...
    url(r'^consumer/(?P<consumer_type>[\w-]+)/edit/(?P<pk>[\d]+)/$', consumer,{"template_name":"consumer/consumer_form.html", "action":"U"},
        name='consumer-edit'),
